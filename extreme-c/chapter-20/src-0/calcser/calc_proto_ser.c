...
bool_t _parse_fields(struct calc_proto_ser_t* ser, char** fields, int field_count, int error_code)
{
...
void _deserialize(struct calc_proto_ser_t* ser, struct buffer_t buff, parse_and_notify_func_t func, int error_code, bool_t* found)
{
    ...
    for (; i < buff.len; i++)
    {
        ...
        if (ser->ring_buf[ser->curr_idx] == MESSAGE_DELIMITER && ser->start_idx >= 0)
        {
            if (found)
            {
                *found = TRUE;
        ...
        else if (ser->ring_buf[ser->curr_idx] != MESSAGE_DELIMITER && ser->start_idx < 0)
        {
...
struct buffer_t calc_proto_ser_server_serialize(struct calc_proto_ser_t* ser, const struct calc_proto_resp_t* resp)
{
    ...
    sprintf(buff.data, "%d%c%d%c%s%c", resp->req_id, FIELD_DELIMITER, (int)resp->status, FIELD_DELIMITER, resp_result_str, MESSAGE_DELIMITER);
